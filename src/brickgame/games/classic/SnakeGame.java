...
final class SnakeGame extends Game {
    ...
    private static final long HEAD_BLINK_PERIOD = 250L;
    ...
    void init(long now) {
        ...
        headVisible = true;
        nextMove = now;
        nextHeadBlink = now + HEAD_BLINK_PERIOD;
        repaint();
    }

    void tick(long now) {
        boolean blinkChanged = false;
        while (now >= nextHeadBlink) {
            nextHeadBlink += HEAD_BLINK_PERIOD;
            headVisible = !headVisible;
            blinkChanged = true;
        }

        if (now >= nextMove) {
            int movePeriod = period();
            do {
                nextMove += movePeriod;
            } while (nextMove <= now);
            onMove(now);
        }
        if (blinkChanged && !engine.isLocked()) {
            repaint();
        }
    }
